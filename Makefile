...
$(STARTER_DIR): packages/elixir_workers/priv/.built
	...
	@sed 's/<%= @app_module %>/Starter/g' \
		packages/elixir_workers/priv/templates/views.ex.eex > $(STARTER_DIR)/lib/starter/views.ex
	@sed 's/<%= @app_module %>/Starter/g' \
		packages/elixir_workers/priv/templates/assets.ex.eex > $(STARTER_DIR)/lib/starter/assets.ex
	@cp packages/elixir_workers/priv/templates/schema.sql $(STARTER_DIR)/schema.sql
	...
	@test -f .dev.vars && cp .dev.vars $(STARTER_DIR)/.dev.vars || true
